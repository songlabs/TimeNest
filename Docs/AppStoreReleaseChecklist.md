...
- [ ] On two physical devices with separate Apple IDs, verify CloudKit invitation acceptance, automatic synchronization of events, shifts, and work records assigned to the owned shared calendar, calendar-name updates, stopping/leaving a share, and excluded private fields. Confirm the CloudKit Production schema and iCloud container are deployed for the submitted build.
- [ ] With owner event editing disabled, confirm received events are read-only. Then enable event editing for a read-write participant and confirm recipient event create/edit/delete; shifts and work records must remain view-only for the recipient.
- [ ] From both owned and received shared-calendar details, verify Copy to My Calendar for All and Specify Period. Confirm inclusive start/end dates, confirmation cancellation, overwrite success, preservation outside the selected range, an empty shared range clearing only the target range, and an incomplete received snapshot blocking without deleting target data.
- [ ] Confirm copied custom shifts keep their color and local times, repeated identical shifts reuse one custom template, editing the personal copy does not change the shared source, and later shared-source changes do not automatically update the personal copy.
